...
queue.on("ready", () => {
  ...
  logger.error(JSON.stringify(error, null, '\t'));
    ...
    logger.info(`job_id: ${id}, status: Job has failed and has no remaining attempts,
    errorMessage: ${errorMessage}`);
})

queue.process("notify", 20, (job: kue.Job, done: any) => {
  Partner.schema.statics.notify(job.data.partnerId, job.data.bidId)
    .then((response: any) => {
...
const notify = (partnerId: String, bidId: String) => {
  if (partnerId && bidId) {
  queue.create("notify", {partnerId: partnerId, bidId: bidId})
    .priority("normal")
    .attempts(5)
    .backoff(true)
